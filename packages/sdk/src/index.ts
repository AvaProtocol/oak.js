...
async function scheduleXcmpTaskWithPayThroughSoverignAccountFlow({
  ...
  const { encodedCallWeight, overallWeight } =
    await destinationChainAdapter.getXcmWeight(
      taskPayloadExtrinsic,
      keyringPair.address,
      oakTransactXcmInstructionCount,
    );
  const scheduleFee = { V3: defaultAsset.location };
  const xcmpFee = await destinationChainAdapter.weightToFee(
    overallWeight,
...
async function scheduleXcmpTaskWithPayThroughRemoteDerivativeAccountFlow({
  ...
  const executionFeeAmout = await destinationChainAdapter.weightToFee(
    overallWeight,
    executionFeeLocation,
  );
  const executionFee = {
  ...
  const {
    encodedCallWeight: taskEncodedCallWeight,
    overallWeight: taskOverallWeight,
  } = await oakAdapter.getXcmWeight(
    taskExtrinsic,
    deriveAccountId,
...
export function Sdk() {
  return {
    ...
    scheduleXcmpTaskWithPayThroughSoverignAccountFlow: async (
      ...
      const result =
        await scheduleXcmpTaskWithPayThroughSoverignAccountFlow(params);
      return result;
    },
    /**
    ...
    scheduleXcmpTaskWithPayThroughRemoteDerivativeAccountFlow: async (
      ...
      const result =
        await scheduleXcmpTaskWithPayThroughRemoteDerivativeAccountFlow(params);
      return result;
    },
  };
}
