...
test(
  "cancel failed with nonexistent taskID",
  async () => {
    ...
    await expect(
      sendExtrinsic(polkadotApi, cancelExtrinsicHex),
    ).rejects.toThrow(`${SECTION_NAME}.TaskDoesNotExist`);
  },
  DEFAULT_TIMEOUT_PER_TEST,
);

test(
  "repeated cancellation of scheduleDynamicDispatchTaskExtrinsic will fail.",
  async () => {
    await checkBalance(polkadotApi, keyringPair);
