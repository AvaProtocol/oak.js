      ...
      ({ status, events, dispatchError }) => {
        if (status?.isFinalized) {
          ...
          const event = _.find(events, ({ event: eventItem }) =>
            polkadotApi.events.system.ExtrinsicSuccess.is(eventItem),
          );
...
export const findExtrinsicFromChain = async (
  ...
  const extrinsic = _.find(
    extrinsics,
    (extrinsicItem) => extrinsicItem.hash.toHex() === extrinsicHash,
  );
  return extrinsic;
};
...
export const cancelTaskAndVerify = async (
  ...
  expect(Buffer.from(taskIdOnChain.toU8a(true)).toString()).toEqual(taskID);
  ...
  expect(
    _.find(
      tasks,
      (task) =>
        !_.isUndefined(_.find(task, (taskItem) => taskItem[1] === taskID)),
    ),
  ).toBeUndefined();
};
...
export const scheduleDynamicDispatchTaskAndVerify = async (
  ...
  if (recurring) {
    ...
    [firstExecutionTime] = executionTimes;
  }
  ...
  const taskScheduledEvent = events.find((record) => {
    const { event } = record;
    return (
      event.method === "TaskScheduled" && event.section === "automationTime"
    );
  });
